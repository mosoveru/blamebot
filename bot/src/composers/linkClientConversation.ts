...
import { BlamebotContext, DatabaseService, ExternalGitSystemDataFetcher, LinkingService } from '@types';
import linkClient from '@conversations';
import provideDatabaseService, { provideFetcher, provideLinker } from '@middlewares';
import ReplyMessages from '@constants';

function buildLinkClientConversation(dbService: DatabaseService) {
  return function (fetcher: ExternalGitSystemDataFetcher) {
    return function (linker: LinkingService) {
      const composer = new Composer<BlamebotContext>();

      composer.use(
        conversations({
          plugins: [provideDatabaseService(dbService), provideFetcher(fetcher), provideLinker(linker)],
        }),
      );
