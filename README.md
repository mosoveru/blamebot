...
Для использования SOCKS прокси в `.env` файле необходимо заполнить поля:

- `USE_PROXY=true` - включаем использование SOCKS прокси в проекте.
- `PROXY_HOSTNAME` - IP-адрес или домен SOCKS прокси
- `PROXY_PORT` - порт SOCKS прокси
- `PROXY_USERNAME` и `PROXY_PASSWORD` - логин и пароль для подключения к SOCKS прокси (необязательно).
